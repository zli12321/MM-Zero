...
# ========================== Code Rendering ========================== #
try:
    from SelfAgent.code_render.render_code import render_single, render_batch_codes
except ImportError:
...
def compute_score(
    ...
    for idx in range(n):
        ...
        if fields is None:
            ...
            # Visual type diversity bonus (escalating penalty for dominance)
            vt = fields.get("visual_type", "matplotlib")
            vt_frac = type_fractions.get(vt, 0.25)
            if vt_frac > 0.8:
                vt_bonus = -VT_DIVERSITY_MAX_BONUS * 2.0
            elif vt_frac > 0.6:
                vt_bonus = -VT_DIVERSITY_MAX_BONUS * 1.5
            else:
