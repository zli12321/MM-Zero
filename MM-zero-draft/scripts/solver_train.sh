...
cleanup_solver_training() {
    ...
    # (Be careful: only kill processes related to our training)
    ...
    echo "[solver_train] Killing all processes using GPU (nvidia-smi)..."
...
_kill_gpu_processes() {
    ...
    pkill -9 -f "ray" 2>/dev/null || true
    pkill -9 -f "verl.trainer.main" 2>/dev/null || true
    echo "[$label] Killing all processes using GPU (nvidia-smi)..."
    for pid in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null); do
        [ -n "$pid" ] && [ "$pid" -gt 0 ] 2>/dev/null && kill -9 "$pid" 2>/dev/null || true
    done
    sleep 5
...
bash SelfAgent/proposal_generate/proposal_generate.bash $proposer_model_path $NUM_PROPOSALS_PER_GPU_SOLVER $experiment_name
...
bash SelfAgent/code_generate/code_generate.bash $codegen_model_path $experiment_name
...
python SelfAgent/code_render/render_code.py --experiment_name $experiment_name --workers $RENDER_WORKERS --timeout 30
...
echo "[Step 4/6] Evaluating rendered images with Solver (8 workers; progress: [0]..[7] chunk X/Y)..."
bash SelfAgent/question_evaluate/evaluate_imagefree.sh $solver_model_path $experiment_name
...
python SelfAgent/question_evaluate/upload_imagefree.py \
    --max_score 0.8 \
    --min_score 0.3 \
...
python3 -m verl.trainer.main \
    config=SelfAgent/configs/imagefree_solver_config_${GPU_MEM}gb${MODEL_SIZE:+_${MODEL_SIZE}}.yaml \
    data.max_response_length=4096 \
    data.train_files=${STORAGE_PATH}/local_parquet/${experiment_name}_train.parquet \
    data.val_files=hiyouga/geometry3k@test \
    data.format_prompt=./SelfAgent/format_prompt/solver.jinja \
    worker.actor.model.model_path=$solver_model_path \
    worker.actor.micro_batch_size_per_device_for_update=1 \
    worker.actor.micro_batch_size_per_device_for_experience=1 \
    worker.reward.reward_function=./SelfAgent/reward_function/cot_val_solver.py:compute_score \
