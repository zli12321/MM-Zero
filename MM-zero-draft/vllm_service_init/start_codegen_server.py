...
This server generates matplotlib code from full proposals (caption, easy_question,
...
parser.add_argument('--chunk_size', type=int, default=16,
                    help='Prompts per generation chunk to avoid OOM (default 16; 16×n=128 sequences).')
...
sample_params = vllm.SamplingParams(
    ...
    n=8,  # Generate 8 code samples per item for robust renderability scoring
)
...
def _build_prompt_text(item: dict) -> str:
    ...
    vt = str(item.get('visual_type', 'matplotlib')).strip().lower()
...
def extract_code_block(response: str, visual_type: str = "matplotlib") -> str:
    """Extract code/SVG from response. For svg: return raw SVG; else Python from ```python ... ```."""
    if visual_type == "svg":
        match = re.search(r"```(?:svg)?\s*\n([\s\S]*?)```", response)
        if match:
            return match.group(1).strip()
        # No block: use full response if it looks like SVG
        s = response.strip()
        if s.lstrip().startswith("<") or "<?xml" in s[:100] or "<svg" in s[:200].lower():
            return s
        return ""
    # Python (matplotlib, plotly, pillow)
...
def codegen():
    ...
    visual_types = [str(item.get('visual_type', 'matplotlib')).strip().lower() for item in data]
    _VALID_VISUAL_TYPES = {'matplotlib', 'plotly', 'pillow', 'svg'}
    for i, vt in enumerate(visual_types):
        if vt not in _VALID_VISUAL_TYPES:
            visual_types[i] = 'matplotlib'
    ...
    for idx in range(len(data)):
        vt = visual_types[idx] if idx < len(visual_types) else "matplotlib"
