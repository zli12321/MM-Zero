...
# Allowed content_type values (must match proposer.jinja and proposer_reward.VALID_CONTENT_TYPES)
_VALID_CONTENT_TYPES = {"data_chart", "diagram", "geometry", "timeline", "map", "table", "other"}


def _normalize_content_type(raw):
    """Normalize content_type to one of the allowed values; return 'other' if invalid/missing."""
    if not raw or not isinstance(raw, str):
        return "other"
    t = raw.strip().lower().replace(" ", "_")
    if t in _VALID_CONTENT_TYPES:
        return t
...
def extract_proposal(response: str):
    ...
    # Extract all fields (content_type is optional; used for diversity in reward)
    content_type_raw = extract_field("content_type", response)
    ...
    # SVG-only mode: always use svg regardless of model output
    visual_type = "svg"
