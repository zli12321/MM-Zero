...
# 5. Easy Question Evaluation
print(f"[{args.suffix}] EVALUATE: Starting easy question evaluation ({args.num_samples} rollouts)...", flush=True)
...
easy_prompts = [
    ...
    for eq in easy_questions
]

easy_valid_chats = []
easy_valid_indices = []
for idx, (prompt, img, eq) in enumerate(zip(easy_prompts, images_pil, easy_questions)):
    if img is not None and eq:
        easy_valid_chats.append({
            "prompt": prompt,
            "multi_modal_data": {"image": img}
        })
        easy_valid_indices.append(idx)

print(f"[{args.suffix}] EVALUATE: {len(easy_valid_chats)} items for easy question evaluation.", flush=True)

easy_responses, easy_valid_indices = _generate_chunked(
    model, easy_valid_chats, easy_valid_indices, sample_params,
    chunk_size=CHUNK_SIZE, suffix=args.suffix, use_tqdm=use_tqdm
)

easy_consistency_map = {}
for response, orig_idx in zip(easy_responses, easy_valid_indices):
    ea_results = [extract_boxed_content(output.text) for output in response.outputs]
    ea_results = [res for res in ea_results if res]
    if not ea_results:
        easy_consistency_map[orig_idx] = 0.0
        continue
    answer_counts = {}
    ...
    if answer_counts:
        ...
        easy_consistency_map[orig_idx] = max_count / len(ea_results)
    else:
        easy_consistency_map[orig_idx] = 0.0

print(f"[{args.suffix}] EVALUATE: Easy question evaluation complete.", flush=True)

for result in results_all:
    oidx = result.pop("_orig_idx")
    result["easy_consistency"] = easy_consistency_map.get(oidx, 0.0)
