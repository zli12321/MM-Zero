...
The Proposer generates: <content_type>, <caption>, <easy_question>, <easy_answer>, <hard_question>, <hard_answer>.
...
Reward computation (per proposal):
  ...
  If format is invalid (can't parse the 5 required tags caption/easy_question/easy_answer/hard_question/hard_answer): reward = -1.0.
  Optional <content_type> is used for diversity; if missing, type is inferred from the caption.

Services (launched by start_proposer_services.sh):
  - CodeGen: 2 instances (GPUs 2-3), n=8 code samples per item
  - Solver:  4 instances (GPUs 4-7), n=10 rollouts per item (hard Q is the bottleneck)
"""
...
# ========================== Cross-Step Caption Memory ========================== #
# Stores captions from recent steps so we can penalize proposals that repeat
# patterns the model has already exploited in previous training steps.
_CAPTION_MEMORY_MAX = int(os.environ.get("CAPTION_MEMORY_MAX_STEPS", "5"))
_caption_history: List[List[str]] = []  # list of (list of captions per step)


def _record_captions(captions: List[str]) -> None:
    """Save this step's captions to history; keep only the last N steps."""
    _caption_history.append(captions)
    while len(_caption_history) > _CAPTION_MEMORY_MAX:
        _caption_history.pop(0)


def _cross_step_similarity(caption: str) -> float:
    """Compute max BLEU similarity between `caption` and all captions from previous steps.
    Returns 0.0 if no history, up to ~1.0 if near-duplicate found."""
    if not _caption_history:
        return 0.0
    from nltk.translate.bleu_score import sentence_bleu, SmoothingFunction
    smoother = SmoothingFunction().method1
    hyp = caption.split()
    max_sim = 0.0
    for past_step in _caption_history:
        for past_cap in past_step:
            ref = [past_cap.split()]
            sim = sentence_bleu(ref, hyp, smoothing_function=smoother)
            if sim > max_sim:
                max_sim = sim
                if max_sim > 0.9:
                    return max_sim
...
# Service port configuration: 2 CodeGen (GPUs 2-3) + 4 Solver (GPUs 4-7). Solver (hard Q) is the bottleneck.
# Set by start_proposer_services.sh in proposer_service_ports.env.
def _get_ports_from_env_or_file(prefix: str, defaults: list[int]) -> list[int]:
    ports = []
    for i in range(len(defaults)):
        v = os.environ.get(f"{prefix}_PORT_{i}")
        if v is not None:
            ports.append(int(v))
    if len(ports) == len(defaults):
        return ports
        ...
        try:
            with open(ports_file) as f:
                for line in f:
                    line = line.strip()
                    for i in range(len(defaults)):
                        key = f"export {prefix}_PORT_{i}="
                        if line.startswith(key):
                            found[i] = int(line.split("=", 1)[1].strip().strip("'\""))
            if len(found) == len(defaults):
                return [found[i] for i in range(len(defaults))]
        except Exception:
            pass
...
SOLVER_PORTS = _get_ports_from_env_or_file("SOLVER", [6010, 6011, 6012])


# ========================== Tag Extraction ========================== #

# Allowed content_type values (from proposer prompt); used for diversity and normalization.
VALID_CONTENT_TYPES = {"data_chart", "diagram", "geometry", "timeline", "map", "table", "other"}


def _normalize_content_type(raw: Optional[str]) -> str:
    """Normalize model output to one of VALID_CONTENT_TYPES; return 'other' if invalid/missing."""
    if not raw or not isinstance(raw, str):
        return "other"
    t = raw.strip().lower().replace(" ", "_")
    if t in VALID_CONTENT_TYPES:
        return t
    # Allow common variants
    if t in ("data chart", "datachart"):
        return "data_chart"
    return "other"


def extract_fields(predict: str) -> Optional[Dict[str, str]]:
    """Extract all structured fields from the Proposer output (caption, content_type, etc.)."""
    ...
    content_type_tag = re.search(r"<content_type>([\s\S]*?)</content_type>", predict)

    if caption and easy_q and easy_a and hard_q and hard_a:
        raw_ct = content_type_tag.group(1).strip() if content_type_tag else ""
        return {
            "visual_type": "svg",
...
def _content_type_from_caption(caption: str) -> str:
    """Classify caption into a coarse content type for diversity (SVG-only: no visual_type, use content type)."""
    if not caption or not isinstance(caption, str):
        return "other"
    c = caption.lower()
    # Order matters: more specific first
    if any(k in c for k in ("flowchart", "flow chart", "network diagram", "org chart", "state diagram", "circuit diagram", "block diagram")):
        return "diagram"
    if any(k in c for k in ("bar chart", "line chart", "pie chart", "scatter", "histogram", "area chart", "stacked", "heatmap", "treemap", "radar chart", "bubble chart")):
        return "data_chart"
    if any(k in c for k in ("angle", "triangle", "circle", "radius", "coordinate", "geometry", "perpendicular", "chord", "polygon", "rectangle")):
        return "geometry"
    if any(k in c for k in ("timeline", "schedule", "gantt")):
        return "timeline"
    if any(k in c for k in ("map", "region", "geographic", "country")):
        return "map"
    if any(k in c for k in ("table", "grid", "comparison table")):
        return "table"
    return "other"
...
def compute_score(
    ...
    CAPTION_DIVERSITY_WEIGHT = float(os.environ.get("CAPTION_DIVERSITY_WEIGHT", "0.5"))
    ...
    # --- 5b2: Content-type diversity (use parsed <content_type> from proposer; fallback to caption-based inference) ---
    # Penalize proposals whose content type is over-represented in the batch.
    # If all rollouts match one type, we skip penalty — no diversity signal to reward.
    CONTENT_TYPE_DIVERSITY_WEIGHT = float(os.environ.get("CONTENT_TYPE_DIVERSITY_WEIGHT", "0.15"))
    CONTENT_TYPE_OVER_REP_THRESHOLD = float(os.environ.get("CONTENT_TYPE_OVER_REP_THRESHOLD", "0.5"))
    content_type_penalties = {}
    if n_valid > 1 and CONTENT_TYPE_DIVERSITY_WEIGHT > 0:
        def _get_content_type(fields: Dict) -> str:
            raw = (fields or {}).get("content_type") or ""
            norm = _normalize_content_type(raw)
            return norm if norm != "other" else _content_type_from_caption((fields or {}).get("caption") or "")
        content_types = [_get_content_type(all_fields[idx]) for idx in valid_indices]
        ct_counts = Counter(content_types)
        ct_frac = {ct: cnt / n_valid for ct, cnt in ct_counts.items()}
        print(f'[proposer_reward] Content-type distribution: {dict(ct_counts)} (fractions: { {k: f"{v:.2f}" for k, v in ct_frac.items()} })')
        if len(ct_frac) > 1:
            # Only apply penalty when we have multiple types; else no diversity signal (e.g. all "other")
            for i, idx in enumerate(valid_indices):
                ct = content_types[i]
                frac = ct_frac.get(ct, 0.0)
                excess = max(0.0, frac - CONTENT_TYPE_OVER_REP_THRESHOLD)
                if excess > 0:
                    scale = excess / (1.0 - CONTENT_TYPE_OVER_REP_THRESHOLD) if CONTENT_TYPE_OVER_REP_THRESHOLD < 1.0 else 1.0
                    content_type_penalties[idx] = CONTENT_TYPE_DIVERSITY_WEIGHT * scale
                else:
                    content_type_penalties[idx] = 0.0
        else:
            print(f'[proposer_reward] Content-type diversity skipped: only one type in batch (no over-representation to penalize)')
        # ensure all valid indices have a value (default 0 when skipped)
        for idx in valid_indices:
            content_type_penalties.setdefault(idx, 0.0)
    else:
        content_type_penalties = {idx: 0.0 for idx in valid_indices}

    # --- 5c: Cross-step caption memory (Fix 5) ---
    # Penalize proposals that are similar to captions from previous training steps.
    # Set CROSS_STEP_PENALTY_WEIGHT=0 (default) to fully disable (no compute, no memory).
    CROSS_STEP_WEIGHT = float(os.environ.get("CROSS_STEP_PENALTY_WEIGHT", "0.0"))
    cross_step_penalties = {}
    if CROSS_STEP_WEIGHT > 0:
        if _caption_history and n_valid > 0:
            print(f'[proposer_reward] Cross-step memory: comparing against {len(_caption_history)} previous steps '
                  f'({sum(len(s) for s in _caption_history)} total past captions)')
            for idx in valid_indices:
                sim = _cross_step_similarity(all_fields[idx]['caption'])
                cross_step_penalties[idx] = sim * CROSS_STEP_WEIGHT
            avg_xstep = np.mean(list(cross_step_penalties.values())) if cross_step_penalties else 0.0
            print(f'[proposer_reward] Cross-step avg similarity penalty: {avg_xstep:.3f}')
        else:
            print(f'[proposer_reward] Cross-step memory: no history yet (first step)')
        step_captions = [all_fields[idx]['caption'] for idx in valid_indices]
        _record_captions(step_captions)
    else:
        print(f'[proposer_reward] Cross-step memory: disabled (CROSS_STEP_PENALTY_WEIGHT=0)')

    # --- 5d: Compute final scores (with Fixes 1, 2) ---
    # Fix 1: EASY_HARD_PENALTY — if avg difficulty < threshold, subtract penalty
    EASY_HARD_THRESHOLD = float(os.environ.get("EASY_HARD_THRESHOLD", "0.15"))
    EASY_HARD_PENALTY = float(os.environ.get("EASY_HARD_PENALTY", "0.3"))
    # Fix 2: SOLVABILITY_CAP — cap solvability contribution per rollout
    SOLVABILITY_CAP = float(os.environ.get("SOLVABILITY_CAP", "0.5"))

    scores = []
    n_easy_hard_penalized = 0
    n_solv_capped = 0
    for idx in range(n):
        fields = all_fields[idx]
        ...
        if fields is None:
            ...
            # Fix 2: cap each solvability score at SOLVABILITY_CAP
            contribution_sum = 0.0
            for solv, diff in zip(easy_scores, hard_scores):
                capped_solv = min(solv, SOLVABILITY_CAP)
                if solv > SOLVABILITY_CAP:
                    n_solv_capped += 1
                contribution_sum += capped_solv + diff
            final_score = contribution_sum / N

            # Fix 1: penalize easy hard questions
            avg_diff = np.mean(hard_scores) if hard_scores else 0.0
            eh_penalty = 0.0
            if avg_diff < EASY_HARD_THRESHOLD and hard_scores:
                eh_penalty = -EASY_HARD_PENALTY
                n_easy_hard_penalized += 1
            final_score += eh_penalty

            # Visual type diversity: SVG-only pipeline — only one type, so no bonus/penalty
            vt_bonus = 0.0
            final_score += vt_bonus

            # Content-type diversity: penalize over-represented caption-derived types (chart/diagram/geometry/...)
            ct_pen = -content_type_penalties.get(idx, 0.0)
            final_score += ct_pen
            ...
            # Fix 5: cross-step caption memory penalty
            xstep_pen = -cross_step_penalties.get(idx, 0.0)
            final_score += xstep_pen
            ...
            scores.append({
                ...
                "easy_hard_penalty": float(eh_penalty),
                "vt_diversity_bonus": float(vt_bonus),
                "content_type_diversity_bonus": float(ct_pen),
                "caption_diversity_bonus": float(cap_div),
                "cross_step_penalty": float(xstep_pen),
            })
    ...
    if valid:
        ...
        print(f'  avg_solvability:  {np.mean([s["avg_solvability"] for s in valid]):.3f}  (capped at {SOLVABILITY_CAP}, {n_solv_capped} rollouts capped)')
        print(f'  avg_difficulty:   {np.mean([s["avg_difficulty"] for s in valid]):.3f}  (threshold={EASY_HARD_THRESHOLD}, {n_easy_hard_penalized}/{len(valid)} penalized -{EASY_HARD_PENALTY})')
        print(f'  avg_vt_bonus:     {np.mean([s["vt_diversity_bonus"] for s in valid]):.3f}')
        print(f'  avg_content_type: {np.mean([s["content_type_diversity_bonus"] for s in valid]):.3f}  (weight={CONTENT_TYPE_DIVERSITY_WEIGHT})')
        print(f'  avg_caption_div:  {np.mean([s["caption_diversity_bonus"] for s in valid]):.3f}  (weight={CAPTION_DIVERSITY_WEIGHT})')
        print(f'  avg_xstep_pen:    {np.mean([s["cross_step_penalty"] for s in valid]):.3f}  (history={len(_caption_history)} steps)')
