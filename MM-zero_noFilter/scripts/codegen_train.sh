...
#
# Pipeline (no filtering):
#   1. Generate proposals from the Proposer
#   2. Convert proposals to a training parquet (keep all valid proposals)
#   3. Launch Solver vLLM service
#   4. Train CodeGen with GRPO (reward = renderability + solvability + difficulty)
#
...
# Step 2: Convert proposals to parquet for GRPO training (no render filtering)
...
echo "[Step 3/4] Cleaning GPUs 2-7 before starting Solver services..."
pkill -9 -f "proposal_generate\.py" 2>/dev/null || true
pkill -9 -f "python.*-m vllm" 2>/dev/null || true
pkill -9 -f "vllm\.entrypoints" 2>/dev/null || true
pkill -9 -f "VLLM::EngineCore" 2>/dev/null || true
pkill -9 -f "start_vllm_server\.py" 2>/dev/null || true
echo "[Step 3/4] Killing GPU compute processes (nvidia-smi)..."
for pid in $(nvidia-smi --query-compute-apps=pid --format=csv,noheader 2>/dev/null); do
    [ -n "$pid" ] && [ "$pid" -gt 0 ] 2>/dev/null && kill -9 "$pid" 2>/dev/null || true
done
sleep 10
...
# Step 4: Train CodeGen with GRPO on GPUs 0-3 (config by GPU_MEM: 40gb or 80gb)
