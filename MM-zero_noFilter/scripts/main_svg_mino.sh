...
#
# Usage: bash SelfAgent_svg_noFilter/scripts/main_svg_mino.sh
# =============================================================================
...
export STORAGE_PATH="${STORAGE_PATH:-/workspace/selfAgent_Storage_svg_long_MIMO_round1_filter}"
export NUM_PROPOSALS_PER_GPU="${NUM_PROPOSALS_PER_GPU:-750}"
...
Base_model="${Base_model:-XiaomiMiMo/MiMo-VL-7B-SFT-2508}"
Model_abbr=Mimo-VL-7B-Instruct-ImageFree-SVG
...
if [ -d "${STORAGE_PATH}/models/${Model_abbr}_codegen_v1/global_step_${TRAIN_STEPS}/actor/huggingface" ]; then
    ...
    bash SelfAgent_svg_noFilter/scripts/codegen_train.sh \
        $Base_model \
...
if [ -d "${STORAGE_PATH}/models/${Model_abbr}_solver_v1/global_step_${TRAIN_STEPS}/actor/huggingface" ]; then
    ...
    bash SelfAgent_svg_noFilter/scripts/solver_train.sh \
        $Base_model \
        ${STORAGE_PATH}/models/${Model_abbr}_proposer_v1/global_step_${TRAIN_STEPS}/actor/huggingface \
