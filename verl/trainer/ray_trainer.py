...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        # get token level scores asynchronously (may block here if reward is slow, e.g. external CodeGen/Solver)
                        print("[trainer] Waiting for reward (CodeGen/Solver/render can take several minutes)...", flush=True)
        ...
        # perform validation after training (skip when val_freq <= 0)
